...
```{r}
drug_deaths <- full_join(drug_deaths, df)
```
...
```{r}
number_of_drug_deaths_plot <- 
  filter(drug_deaths, number_of_drug_deaths != "NA") |> 
  select(cause_of_death, number_of_drug_deaths)
```

```{r}
ggplot(number_of_drug_deaths_plot,
       aes(x = number_of_drug_deaths)) +
  geom_histogram()
```
